...
const judulBlogs = document.querySelectorAll("#judulBlog");
...
inputBox.addEventListener("input", (e) => {
  ...
  // Loop through all the a elements and check if the text matches
  judulBlogs.forEach((element) => {
  ...
  // Clear the result box and append the matching elements (up to 5 elements)
  resultBox.innerHTML = "";
  const maxResults = Math.min(matchingElements.length, 5);
  for (let i = 0; i < maxResults; i++) {
    resultBox.appendChild(matchingElements[i].cloneNode(true));
  }
});

// Blog sistem
const showAllBlogButton = document.getElementById("showAllBlog");
const allBlogDiv = document.getElementById("allBlog");

let isShowingAll = false;

showAllBlogButton.addEventListener("click", () => {
  isShowingAll = !isShowingAll;
  allBlogDiv.classList.toggle("show-all", isShowingAll);
  showAllBlogButton.textContent = isShowingAll ? "Tutup Semua blog" : "Tampilkan semua blog";
});
